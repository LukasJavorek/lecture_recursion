...
def recursive_binary_search(list, value, left, right):

    if list[(right + left) // 2] == value:
        return (right + left) // 2 + 1

    elif right <= left:
        return None

    elif list[(right + left) // 2] > value:
        right = (right + left) // 2 - 1
        return recursive_binary_search(list, value, left, right)
